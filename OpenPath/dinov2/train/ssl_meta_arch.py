...
class SSLMetaArch(nn.Module):
    def __init__(self, cfg):
        ...
        if self.do_gram:
            from dinov2.loss import GramLoss
            _, gram_teacher_backbone, _ = build_model_from_cfg(cfg)
            self.gram_teacher = nn.ModuleDict({"backbone": gram_teacher_backbone})
            self.gram_teacher.requires_grad_(False)
            self.gram_loss = GramLoss(
                apply_norm=cfg.gram.get("normalized", True),
                remove_neg=cfg.gram.get("remove_neg", True),
            )
            self.gram_loss_weight = cfg.gram.loss_weight
            self.gram_it_first_update = int(cfg.gram.it_first_update)
            self.gram_ckpt = cfg.gram.ckpt
            # 앵커 가중치 로드(teacher_checkpoint의 backbone.* 추출) — FSDP 전(plain module)에서.
            ck = torch.load(self.gram_ckpt, map_location="cpu", weights_only=False)
            t = ck["teacher"] if "teacher" in ck else ck
            sd = {k[len("backbone."):]: v for k, v in t.items() if k.startswith("backbone.")}
            miss, unexp = self.gram_teacher["backbone"].load_state_dict(sd, strict=False)
            logger.info(f"GRAM anchor loaded from {self.gram_ckpt}: missing={len(miss)} unexpected={len(unexp)}, "
                        f"it_first_update={self.gram_it_first_update} weight={self.gram_loss_weight}")
    ...
    def forward_backward(self, images, teacher_temp, gram_weight=0.0):
        n_global_crops = 2
        ...
        # ---- Gram anchoring loss (it_first_update 이후 gram_weight>0일 때만) ----
        # global crop의 student patch 구조를 얼린 앵커(peak)와 가깝게 유지 → 후반 degradation 방지.
        if self.do_gram and gram_weight > 0:
            with torch.no_grad():
                gram_teacher_patch = self.gram_teacher["backbone"](global_crops, is_training=True)["x_norm_patchtokens"]
            reshard_fsdp_model(self.gram_teacher)
            gram_student_patch = student_global_backbone_output_dict["x_norm_patchtokens"]
            gram_loss = self.gram_loss(gram_student_patch, gram_teacher_patch, img_level=True)
            loss_dict["gram_loss"] = gram_loss
            loss_accumulator = loss_accumulator + gram_weight * gram_loss
    ...
    def prepare_for_distributed_training(self):
        ...
        # ★ gram_teacher: 앵커 가중치 유지(student 동기화 안 함) + 얼린 채 FSDP wrap.
        if self.do_gram:
            gram_cfg = self.cfg.compute_precision.teacher["backbone"]
            self.gram_teacher["backbone"] = get_fsdp_wrapper(gram_cfg, modules_to_wrap={BlockChunk})(
                self.gram_teacher["backbone"]
            )
